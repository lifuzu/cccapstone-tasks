...
def main(ssc):
    ...
    def process(time, rdd):
        ...
        try:
            ...
            delays= parts.map(lambda w: Row(origin=w[0], dest=w[1], delay=float(w[2])))
            dataFrame = sqlContext.createDataFrame(delays)
            # Register as table
            dataFrame.registerTempTable("origin_dest_delays")
            # Do word count on table using SQL and print it
            delays_df = \
                sqlContext.sql("SELECT origin, dest, avg(delay) AS avg_delay FROM origin_dest_delays GROUP BY origin, dest")
            ...
            delays_df.write \
                ...
                .options( table = "task2_part2_group2_2", keyspace = "mykeyspace") \
                .save(mode="append")
            #delays_df.show()
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: kafka_group2_2.py <zk> <topic>", file=sys.stderr)
        exit(-1)
